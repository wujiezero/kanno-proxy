#!/bin/sh
# TomFly - generate mihomo config.yaml from UCI
...
generate_mihomo_config() {
    ...
    [ -n "${TOMFLY_FORCE_TPROXY:-}" ] && tun=0   # runtime fallback when TUN can't initialise
    local dns_mode=$(_u "dns.mode"); dns_mode="${dns_mode:-fake-ip}"
    local dns_port=$(_u "dns.listen_port"); dns_port="${dns_port:-1053}"

    cat <<YAML
# Generated by TomFly $(date '+%Y-%m-%d %H:%M:%S') - DO NOT EDIT
mixed-port: 7890
tproxy-port: 7893
redir-port: 7892
allow-lan: true
...
dns:
  ...
  nameserver:
YAML
    uci -q get tomfly.dns.domestic_dns 2>/dev/null | tr ' ' '\n' | while read -r ns; do
        [ -n "$ns" ] && printf '    - %s\n' "$ns"
    done
    cat <<YAML
  fallback:
YAML
    uci -q get tomfly.dns.foreign_dns 2>/dev/null | tr ' ' '\n' | while read -r ns; do
        [ -n "$ns" ] && printf '    - "tls://%s"\n' "$ns"
    done
    cat <<YAML
  fallback-filter:
...
YAML
    ...
    : > /tmp/tomfly_usable.tmp
    uci show tomfly 2>/dev/null | grep "^tomfly\.proxy_.*\.type=" | while read -r line; do
        ...
        local reason
        if reason=$(tomfly_node_incompat mihomo "$sec"); then
            echo "$sec" >> /tmp/tomfly_usable.tmp
        else
